...
def custom_reverse(input_list):
    ...
    #the following only reverses a copy
    #return input_list[::-1]

    #loop through each item in the list
    for i in range(custom_len(input_list)+1):
        #pop the last item of the list ands stores as a value
        last_item = custom_pop(input_list)
        #insert it into the first position
        custom_insert(input_list, i, last_item)

def custom_contains(input_list, value):
    """custom_contains(input_list, value) imitates (value in input_list)"""
    # loop through each item in the list
    # check to see if value is equal to the item
    for i in input_list:
        if i == value:
            return True
